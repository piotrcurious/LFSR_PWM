...
# --- LFSR CORE FUNCTION (Fibonacci with Failsafe) ---

def run_lfsr_sequence(mask, seed, level, sequence_len, width=16):
    """
    Runs a deterministic 16-bit LFSR using the classic Fibonacci (External XOR) structure
    and incorporates the self-healing failsafe mechanism to prevent state degeneration.
    
    The LFSR state's MSB (most significant 8 bits) is compared against 
    the PWM 'level' (0-255) to generate the deterministic PWM signal.
    """
    
    ALL_ONES = (1 << width) - 1 # 0xFFFF for 16 bits

    # Initial state.
    state = seed 
    signal = []
    
    # We only need sequence_len bits, but we run the LFSR for the whole length.
    for k in range(sequence_len):
        ...
        # --- 2. LFSR Failsafe and Update (Fibonacci Type) ---
        
        # Failsafe check (Self-Healing: if state degenerates to 0 or ALL_ONES, re-seed)
        if state == 0 or state == ALL_ONES:
            # Uses the original LUT seed as the failsafe_seed (the restart logic)
            state = seed 
            
        else:
            # Standard Fibonacci (External XOR) LFSR Update:
            
            # 2a. Determine feedback (LSB is the output bit)
            lfsr_out = state & 1 
            
            # 2b. Shift the state right
            state >>= 1 
            
            # 2c. Conditionally apply the feedback mask
            if lfsr_out:
                state ^= mask 
            
        # Ensure state remains a 16-bit integer (always do this last)
        state &= ALL_ONES 
...
def generate_lfsr_pwm_signal(level, sequence_len):
    ...
    lfsr_signal = run_lfsr_sequence(mask, seed, level, sequence_len)
...
class PWM_Analyzer_App:
    ...
    def update_plots(self, level_val):
        ...
        # --- 1. Signal Generation and Calculation ---
