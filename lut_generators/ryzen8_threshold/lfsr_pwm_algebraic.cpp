// lfsr_pwm_threshold_search_fixed.cpp
// LFSR PWM using state-threshold comparison where seed doubles as threshold.
// Fixed: consider every state in each cycle as a candidate threshold and
//        sample PWM output bits (state > threshold) for Berlekamp–Massey.
// Build: g++ -std=c++17 -O3 -march=native -flto -funroll-loops -o lfsr_pwm_threshold_search_fixed lfsr_pwm_threshold_search_fixed.cpp
...
// This version emits a CycleCandidate for every state inside each detected cycle.
struct CycleCandidate {
    u64 start_state;
    int period;
    double duty_cycle;
    std::vector<uint8_t> sample_bits; // PWM output bits (state > threshold)
};
...
std::vector<CycleCandidate> find_all_cycles(u64 mask, int n) {
    ...
    for (u64 s = 1; s < max_state; ++s) {
        ...
        // Produce a candidate for every state in the cycle (every possible threshold)
        int sample_len = std::min(256, (int)cycle_len);
        for (size_t j = 0; j < cycle_len; ++j) {
            CycleCandidate cand;
            cand.start_state = cycle[j];
            cand.period = (int)cycle_len;

            // compute duty by scanning the cycle once
            int ones = 0;
            for (size_t k = 0; k < cycle_len; ++k) {
                u64 st = cycle[(j + k) % cycle_len];
                if (st > cand.start_state) ones++;
            }
            cand.duty_cycle = (double)ones / (double)cand.period;

            // sample the PWM output bits (not the LSB)
            cand.sample_bits.clear();
            cand.sample_bits.reserve(sample_len);
            for (int k = 0; k < sample_len; ++k) {
                u64 st = cycle[(j + k) % cycle_len];
                cand.sample_bits.push_back((uint8_t)(st > cand.start_state ? 1 : 0));
            }
...
// Uses LC caching and safe pruning based on maximum possible bonus to avoid unnecessary BM runs.
void evaluate_mask_for_all_levels(u64 mask, int n, const std::vector<double> &targets,
    ...
    // LC cache keyed by FNV1a hash of sample_bits
    static thread_local std::unordered_map<uint64_t,int> lc_cache;
    // Compute an upper bound on bonuses to use for safe pruning:
    // lc_bonus <= lc_weight * (lc_max / n) <= lc_weight * 1
    // period_bonus <= period_weight * (log2(max_period) / n) <= period_weight * (n / n) = period_weight
    double max_bonus = lc_weight + period_weight;
    ...
    for (const auto &cand : candidates) {
        // Precompute log2(period)/n term
        double period_term = (cand.period > 0) ? (std::log2((double)cand.period) / (double)n) : 0.0;
        
        // Compute hash for sample_bits once (for possible LC caching)
        uint64_t bits_hash = fnv1a_hash_bits(cand.sample_bits);
        
        for (size_t i = 0; i < targets.size(); ++i) {
            double target = targets[i];
            double error = fabs(cand.duty_cycle - target);
            
            // Safe pruning: even with best bonuses, the minimal possible score for this candidate
            // is error - max_bonus. If that minimum >= current best score, skip.
            double min_possible_score = error - max_bonus;
            if (min_possible_score >= local_configs[i].score) continue;
            
            // Compute LC (cached)
            int cand_lc;
            auto it = lc_cache.find(bits_hash);
            if (it != lc_cache.end()) {
                cand_lc = it->second;
            } else {
                cand_lc = berlekamp_massey(cand.sample_bits);
                lc_cache.emplace(bits_hash, cand_lc);
            }
...
int main(int argc, char **argv) {
    ...
    std::cerr << "LFSR PWM Threshold Search (seed doubles as threshold) - fixed\n";
