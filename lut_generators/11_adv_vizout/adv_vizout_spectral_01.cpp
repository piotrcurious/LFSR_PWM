    ...
    for (const auto &cycle : cycles) {
        ...
        for (size_t ti = 0; ti < targets.size(); ++ti) {
            ...
            for (int kc = 0; kc < k_count; ++kc) {
                ...
                double lf_energy = compute_low_freq_energy(ones_flags, period, 5);
                ...
                if (score + EPS < local_configs[ti].score) {
                    ...
                    local_configs[ti].low_freq_energy = lf_energy;
                }
            } // kc
        } // targets
    } // cycles
    ...
    if (n <= 16) {
        ...
        for (size_t i = 0; i < configs.size(); ++i) {
            ...
            f << std::dec << ", // " << std::setw(3) << i
              ...
              << " LF=" << std::scientific << std::setprecision(2) << c.low_freq_energy << "\n";
        }
        f << "};\n\n";
