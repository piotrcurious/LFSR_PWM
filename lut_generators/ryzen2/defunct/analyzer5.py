...
class PWM_Analyzer_App:
    ...
    def on_mouse_wheel(self, event):
        ...
        # The update_plots is called automatically by the variable change, but explicitly calling it ensures consistency
    ...
    def setup_analyzer_ui(self):
        ...
        # Main frame to hold plots and slider side-by-side
        main_analysis_frame = ttk.Frame(self.master)
        main_analysis_frame.pack(side=tk.TOP, fill=tk.BOTH, expand=1, padx=10, pady=10)

        # Left side: Plots
        self.canvas_widget.pack(in_=main_analysis_frame, side=tk.LEFT, fill=tk.BOTH, expand=1, padx=(0, 10))

        # Right side: Vertical Slider and Info
        slider_frame = ttk.Frame(main_analysis_frame, padding="10")
        slider_frame.pack(in_=main_analysis_frame, side=tk.RIGHT, fill=tk.Y)
        ...
        # Vertical Slider (Inverted: 255 at top, 0 at bottom)
