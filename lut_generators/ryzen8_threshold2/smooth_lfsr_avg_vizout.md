...
struct Options {
    ...
    int block_size = 256; // window size
    double max_block_error = 1.0; // allowed max error per window (1.0 = effectively off)
...
void prepare_output_files(const std::string &dir) {
    ...
    for (int i = 0; i < 256; ++i) {
        ...
        // open for append (so multiple runs append if desired); truncate to start fresh
...
void close_output_files() {
    for (int fd : out_fds) if (fd >= 0) close(fd);
    out_fds.clear();
    file_mutexes.clear();
}

// Helper to export a candidate to a particular PWM bin index (0..255)
...
int main(int argc, char **argv) {
    ...
    // min_sample_bits already set in parse_args; ensure reasonable cap
...
Thanks — I rethought the numerical/phase-instability issues and pushed an improved, self-contained C++ file into the canvas (named Lfsr Pwm Threshold Search Improved Export).

Short summary of the changes I made (what to expect in the code):

Reduced phase/aliasing bias: when checking circular sliding windows I now ensure we sample start positions that cover every residue class modulo g = gcd(period, block_size) at least once. This captures wrap-around alignment differences between LFSR period and window size (the main cause of apparent mismatches for short-period LFSRs).

Deterministic, bounded sampling: keeps the original MAX_WINDOWS_CHECK bound, but the sampled starts are chosen deterministically to evenly cover the relevant alignments instead of naive striding that can miss critical offsets.

Phase-robust LC estimation: instead of computing linear-complexity (Berlekamp–Massey) from a single phase, the code computes LC at a few phase offsets and uses the median LC — this reduces noisy changes due to phase alignment while still being fast and cache-friendly (cached by a hash).

Safer numeric comparisons: added small EPS tolerances to floating comparisons and widened the duty-delta tolerance by tiny epsilon to avoid false rejections due to floating rounding.

Avoid exploding LCMs: the code purposely avoids materializing an LCM-length sequence (which can overflow or be enormous). Instead it uses the residue coverage trick and a bounded prefix array, which is both correct for wrap-around checking and practical for large periods.

Defensive limits: caps on sample lengths and improved min_sample_bits handling to avoid runaway memory/time for very large n.

Minor scoring / heuristic behavior left intact: the LC and period bonuses are preserved and the score still prefers low block error with bonuses for LC/period; you can tune lc_weight / period_weight as before.


I placed the full updated file in the canvas so you can open/download/run it directly. If you want I can:

Run a focused micro-benchmark or unit test (small n, a few masks) and show before/after pass/fail counts for a specific block_size / resolution you care about.

Further reduce aliasing by explicitly simulating a fixed wall-clock evaluation window (LCM-capped) for small periods if you'd like absolute time-domain correctness at the cost of heavier computation.

Tune the LC-phase count or sampling strategy for your preferred tradeoff between speed and robustness.


If you'd like any of those follow-ups, tell me which direction and I’ll extend the code in-place (or add a small test harness).
