...
// candidate_counts is incremented for each target when a threshold candidate passes the sliding-window constraints
        ...
        for (size_t ti = 0; ti < targets.size(); ++ti) {
            ...
            for (int kc = 0; kc < k_count; ++kc) {
                ...
                if (ones_count_period != k) {
                    // update duty and error accordingly (safety)
                ...
                // we need prefix up to period + block_size (start in [0,period-1], end = start+block_size)
                ...
                // For circular sliding windows we consider starts at s in [0, period-1], optionally sampled with stride
                // We'll check start positions: 0, stride, 2*stride, ... until >= period
                for (int s = 0, checked = 0; s < period && checked < check_positions; s += stride, ++checked) {
                    int start_pos = s; // start inside cycle
                    ...
                    if (block_error > max_block_error) {
                        window_reject = true;
                        break;
                    }
                }
                ...
                if (max_block_duty_delta > pwm_bin) {
                    // If we sampled start positions (stride>1), max_block_duty_delta is approximate upper bound for
                    // sampled positions; we conservatively reject when it exceeds the bin.
                    continue;
                }
                ...
                // Build sample bits for LC/hashing. Use repeated cycle bits until sample_len.
...
int main(int argc, char **argv) {
    ...
    std::cerr << "LFSR PWM Threshold Search (improved, circular sliding-window)\n";
