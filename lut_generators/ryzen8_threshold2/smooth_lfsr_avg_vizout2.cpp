// lfsr_pwm_threshold_search_improved_export.cpp // Improved LFSR PWM threshold search with circular sliding-window (wrap-around) error constraint // and export of valid candidates into binary visualization bins. // Changes (summary): //  - Reduce aliasing / phase-bias by ensuring window start coverage across residues modulo gcd(period, block_size). //  - Use small numeric tolerances (epsilon) for floating comparisons. //  - Cap LCM-based expansions and avoid exploding evaluation lengths. //  - Evaluate linear-complexity (Berlekamp–Massey) across a few phase offsets and use the median result to reduce phase bias. //  - Make sampling/stride selection deterministic and ensure we always cover distinct alignments relevant to wrap-around behavior. //  - Minor defensive fixes (includes, caps) and clearer comments. // Build: g++ -std=c++17 -O3 -march=native -flto -funroll-loops -pthread -o lfsr_pwm_threshold_search_improved_export lfsr_pwm_threshold_search_improved_export.cpp

#include <bits/stdc++.h> #include <atomic> #include <thread> #include <mutex> #include <getopt.h> #include <chrono> #include <functional> #include <sys/stat.h> #include <fcntl.h> #include <unistd.h> #include <cerrno> #include <numeric> // for gcd/lcm

using u64 = uint64_t; using u32 = uint32_t;

struct LFSRConfig { u64 mask = 0; u64 seed = 0;      // Doubles as both starting state AND threshold (kept for LUT compatibility) int period = 0; int lc = 0; double actual_duty = 0.0; double error = 0.0; double score = 0.0; };

static inline int popcount_u64(u64 x) { return __builtin_popcountll(x); }

// Berlekamp–Massey over GF(2) to estimate linear complexity int berlekamp_massey(const std::vector<uint8_t> &bits) { int n = (int)bits.size(); if (n == 0) return 0; std::vector<uint8_t> C(n,0), B(n,0); C[0]=1; B[0]=1; int L=0, m=-1; for (int N=0; N<n; ++N) { int d=0; for (int i=0;i<=L;++i) d ^= (C[i] & bits[N-i]); if (d) { std::vector<uint8_t> T = C; int p = N - m; for (int i = 0; i + p < n; ++i) C[i+p] ^= B[i]; if (L <= N/2) { L = N + 1 - L; m = N; B.swap(T); } } } return L; }

// Next state for Fibonacci-style right shift LFSR static inline u64 next_state(u64 state, u64 mask, int n) { u64 fb = __builtin_parityll(state & mask); return (state >> 1) | (fb << (n - 1)); }

// FNV-1a 64-bit hash for a sequence of 0/1 bytes static uint64_t fnv1a_hash_bits(const std::vector<uint8_t> &bits) { const uint64_t FNV_OFFSET = 1469598103934665603ULL; const uint64_t FNV_PRIME  = 1099511628211ULL; uint64_t h = FNV_OFFSET; for (uint8_t b : bits) { h ^= (uint64_t)(b & 1); h *= FNV_PRIME; } return h; }

// Find all cycles for a mask and return vector of cycles (each cycle is vector<u64>) std::vector<std::vector<u64>> find_all_cycles_raw(u64 mask, int n) { const u64 max_state = (1ULL << n); std::vector<uint8_t> visited(max_state, 0); std::vector<std::vector<u64>> cycles;
...
struct Options { int n = 16; int resolution = 256; int threads = 0; u64 max_masks = 0;  // 0 = scan all std::string output = "lfsr_pwm_lut.h"; double lc_weight = 0.01; double period_weight = 0.001; unsigned seed = 0; int min_taps = 2;  // Minimum number of taps (bits set in mask) double min_period_ratio = 0.0; // skip cycles shorter than ratio * (1<<n) int min_sample_bits = 0; // computed from n int block_size = 256; // window size double max_block_error = 1.0; // allowed max error per window (1.0 = effectively off) std::string out_dir = "8bit_set"; bool export_bins = true; };

Options parse_args(int argc, char **argv) { Options opt; static struct option long_options[] = { {"n", required_argument, nullptr, 'n'}, {"res", required_argument, nullptr, 'r'}, {"threads", required_argument, nullptr, 't'}, {"max-masks", required_argument, nullptr, 'm'}, {"output", required_argument, nullptr, 'o'}, {"lc-weight", required_argument, nullptr, 'l'}, {"period-weight", required_argument, nullptr, 'p'}, {"seed", required_argument, nullptr, 's'}, {"min-taps", required_argument, nullptr, 'T'}, {"min-period-ratio", required_argument, nullptr, 'q'}, {"block-size", required_argument, nullptr, 'b'}, {"max-block-error", required_argument, nullptr, 'e'}, {"out-dir", required_argument, nullptr, 'd'}, {"no-export-bins", no_argument, nullptr, 1}, {nullptr, 0, nullptr, 0} };
...
opt.min_sample_bits = std::max(8 * opt.n, 256); // ensure some minimal sampling but not huge

return opt;

}

// ---------------------- Export support ---------------------- static std::vector<int> out_fds;          // one fd per 256 bins static std::vectorstd::mutex file_mutexes; static bool use_16bit_entry = true;

void ensure_directory(const std::string &dir) { struct stat st{}; if (stat(dir.c_str(), &st) == -1) { if (mkdir(dir.c_str(), 0755) != 0) { std::perror("mkdir"); std::exit(1); } } }

void prepare_output_files(const std::string &dir) { ensure_directory(dir); out_fds.assign(256, -1); file_mutexes.resize(256); char path[512]; for (int i = 0; i < 256; ++i) { snprintf(path, sizeof(path), "%s/0x%02X.bin", dir.c_str(), i); // open for append (so multiple runs append if desired); truncate to start fresh int fd = open(path, O_WRONLY | O_CREAT | O_TRUNC, 0644); if (fd < 0) { std::perror("open"); std::exit(1); } out_fds[i] = fd; } }

static ssize_t full_write_fd(int fd, const void* buf, size_t count) { const char* p = static_cast<const char*>(buf); while (count) { ssize_t w = write(fd, p, count); if (w < 0) { if (errno == EINTR) continue; return -1; } p += w; count -= w; } return 0; }

void close_output_files() { for (int fd : out_fds) if (fd >= 0) close(fd); out_fds.clear(); file_mutexes.clear(); }

// Helper to export a candidate to a particular PWM bin index (0..255) inline void export_candidate_bin(int pwm_index, u64 mask, u64 seed) { if (pwm_index < 0 || pwm_index > 255) return; int fd = out_fds[pwm_index]; if (fd < 0) return; std::lock_guardstd::mutex lock(file_mutexes[pwm_index]); if (use_16bit_entry) { uint16_t m16 = static_cast<uint16_t>(mask); uint16_t s16 = static_cast<uint16_t>(seed); struct { uint16_t mask; uint16_t seed; } e; e.mask = m16; e.seed = s16; full_write_fd(fd, &e, sizeof(e)); } else { uint32_t m32 = static_cast<uint32_t>(mask); uint32_t s32 = static_cast<uint32_t>(seed); struct { uint32_t mask; uint32_t seed; } e; e.mask = m32; e.seed = s32; full_write_fd(fd, &e, sizeof(e)); } }

// ---------------------- Evaluation ----------------------

// Evaluate a single mask: for each cycle, for each target compute the best threshold candidate // candidate_counts is used to increment the number of candidates that pass the configured block constraints per target void evaluate_mask_for_all_levels(u64 mask, int n, const std::vector<double> &targets, std::vector<LFSRConfig> &best_configs, std::vector<uint32_t> &candidate_counts, std::mutex &merge_mutex, double lc_weight, double period_weight, double min_period_ratio, int min_sample_bits, int max_sample_bits, int block_size, double max_block_error, int resolution, bool export_bins_flag) { // Find cycles std::vector<std::vector<u64>> cycles = find_all_cycles_raw(mask, n); if (cycles.empty()) return;
...
// If period is larger, we sample start positions with a stride to keep checks bounded.
...
for (const auto &cycle : cycles) {
    ...
    for (size_t ti = 0; ti < targets.size(); ++ti) {
        ...
        for (int kc = 0; kc < k_count; ++kc) {
            ...
            // If ones count per period doesn't match k (shouldn't normally happen), recompute duty
            ...
            int ext_len = period + block_size; // cheap; we avoid constructing huge LCMs
            ...
            if (period > MAX_WINDOWS_CHECK) {
                ...
                stride = (period + check_positions - 1) / check_positions; // ceil
            }

            // To avoid aliasing where short periods align with block boundaries, ensure we cover all residues
            // modulo gcd(period, block_size) at least once. This captures wrap-around alignment differences.
            ...
            starts.reserve(std::min(period, check_positions));
            ...
            for (int r = 0; (int)starts.size() < check_positions && r < period; ++r) {
                int val = (r * g) % period; // deterministic spread
                starts.push_back(val);
            }
            // trim and unique-ify while preserving order
            ...
            // For circular sliding windows we consider the selected start positions
            for (int sidx = 0; sidx < (int)starts.size(); ++sidx) {
                int start_pos = starts[sidx];
                if (start_pos < 0) start_pos += period;
                // Wrap-around uses prefix built above which already handles modulo
            ...
            // allow small epsilon margin when comparing deltas
            ...
            if (export_bins_flag) {
                // Map target index to an 8-bit bin index. For resolution==256 this is exact.
            ...
            // Build sample bits for LC/hashing. Use a few phase offsets and take median LC to reduce bias.
...
// Write Arduino LUT with just mask and seed (seed doubles as threshold) void write_arduino_lut(const std::vector<LFSRConfig> &configs, int n, const std::string &filename) { std::ofstream f(filename); if (!f) { std::cerr << "ERROR: cannot open " << filename << " for writing\n"; return; }
...
int main(int argc, char **argv) { Options opt = parse_args(argc, argv); if (opt.n < 8 || opt.n > 32) { std::cerr << "n should be between 8 and 32 (practical limits)\n"; return 1; } // min_sample_bits already set in parse_args; ensure reasonable cap opt.min_sample_bits = std::clamp(opt.min_sample_bits, 64, 16384);
...
// Candidate counts per target (how many threshold candidates passed the sliding-window constraints)
