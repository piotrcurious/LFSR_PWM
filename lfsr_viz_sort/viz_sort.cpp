...
//
// Compile:
// g++ -O3 -std=c++17 -march=native -o lfsr_analytics_viz lfsr_analytics_viz.cpp -lfltk -lfltk_gl -lGL -lGLU -lpthread
...
EvalResult evaluate_point(const Point2D &p, int bin_id) {
    ...
    uint16_t threshold = p.seed;

    for (uint32_t i = 0; i < N; ++i) {
//        int out = (static_cast<uint32_t>(lfsr_state) > threshold) ? 1 : 0;
        int out = (lfsr_state > threshold) ? 1 : 0;
        if (out) {
            er.total_on++;
...
void evaluate_worker_background() {
    ...
    for (int b = 0; b < 256; ++b) {
        ...
        {
            std::lock_guard<std::mutex> lock(g_data_mutex);
            local_points = g_bins[b].points; // snapshot copy
        }
        ...
        for (size_t i = 0; i < local_points.size(); ++i) {
            ...
            if ((i & 4095) == 0) std::cerr << "."; // coarse progress
        }
...
eval_abort:
    {
        ...
        for (int b = 0; b < 256; ++b) {
            ...
            if (!local_eval[b].empty()) g_eval_by_bin[b] = std::move(local_eval[b]);
        }
    }
...
void start_evaluate_cb(Fl_Widget* w, void* userdata) {
    ...
    g_eval_thread = std::thread([](){ evaluate_worker_background(); });
...
public:
    ...
    void draw() override {
        ...
        struct DrawBin { float r,g,b; int id; std::vector<Point2D> pts; };
        std::vector<DrawBin> drawbins;
        {
            ...
            for (const auto& bin : g_bins) {
                ...
                DrawBin db;
                db.r = bin.r; db.g = bin.g; db.b = bin.b; db.id = bin.id;
    ...
    int handle(int event) override {
        switch (event) {
            case FL_PUSH:
                ...
                return 1;
            case FL_RELEASE:
                drag_rot = false; drag_pan = false; return 1;
            case FL_DRAG: {
                int dx = Fl::event_x() - last_mx;
                int dy = Fl::event_y() - last_my;
                last_mx = Fl::event_x(); last_my = Fl::event_y();
                if (drag_rot) { cam_yaw += dx * 0.5f; cam_pitch += dy * 0.5f; }
                if (drag_pan) {
...
// -----------------------------------------------------------------------------
// OctagonSortView replaced by star-visualization per-LFSR
// -----------------------------------------------------------------------------
...
public:
    ...
    // Draw a star: 8 radial lines and a dot at each outer end.
    // lengths are proportional to section_counts[k] normalized by 65535,
    // and scaled so max length == stack_height/2 (stack_height computed by caller).
    void draw_star_at(float cx, float cy, float cz, const EvalResult &er, float stack_height, float rcol, float gcol, float bcol) {
        const float inv65535 = 1.0f / 65535.0f;
        const float min_frac = 0.02f; // small visible floor
        float max_len = stack_height * 0.5f; // full radius in world units

        // draw radial lines
        glLineWidth(2.0f);
        glBegin(GL_LINES);
        glColor4f(rcol, gcol, bcol, 0.9f);
        for (int k = 0; k < 8; ++k) {
            float frac = static_cast<float>(er.section_counts[k]) * inv65535;
            if (frac < min_frac) frac = min_frac;
            if (frac > 1.0f) frac = 1.0f;
            float len = max_len * frac;
            float ang = 2.0f * 3.14159265358979323846f * (float)k / 8.0f;
            float ex = cx + std::cos(ang) * len;
            float ez = cz + std::sin(ang) * len;
            glVertex3f(cx, cy, cz);
            glVertex3f(ex, cy, ez);
        }
        glEnd();
        ...
        // outer dots
        // set point size proportional to length (clamped)
        float avg_frac = static_cast<float>(er.total_on) / 65535.0f;
        // we draw each dot sized by its own frac to emphasize differences
        for (int k = 0; k < 8; ++k) {
            float frac = static_cast<float>(er.section_counts[k]) * inv65535;
            if (frac < min_frac) frac = min_frac;
            if (frac > 1.0f) frac = 1.0f;
            ...
            // bigger dot for larger frac
            float psize = 2.0f + 8.0f * frac; // range ~2..10
            glPointSize(psize);
            glBegin(GL_POINTS);
            glColor4f(1.0f, 1.0f, 1.0f, 0.95f); // white highlight
            glVertex3f(ex, cy, ez);
            glEnd();

            // inner colored dot for contrast
            glPointSize(std::max(1.5f, psize * 0.6f));
            glBegin(GL_POINTS);
            glColor4f(rcol, gcol, bcol, 1.0f);
            glVertex3f(ex, cy, ez);
            glEnd();
        }
        ...
        glVertex3f(cx, cy, cz);
        glEnd();
        // reset pointsize (some GL implementations require no reset)
        glPointSize(1.0f);
    }
    ...
    void draw() override {
        ...
        // Copy evaluated bins under lock
        ...
        std::vector<BinCopy> bins;
        {
            ...
            for (int b=0;b<256;++b) {
                if (!g_eval_by_bin[b].empty()) {
                    BinCopy bc;
                    bc.bin_id = b;
                    bc.entries = g_eval_by_bin[b]; // copy vector of EvalResult
        ...
        // For each bin, compute stack layout, and draw star for each LFSR stacked vertically
        for (int idx = 0; idx < nbins; ++idx) {
            ...
            for (int e = 0; e < n; ++e) {
                ...
                draw_star_at(center_x, cy, center_z, er, stack_height, rr, gg, bb);
            }
        }
    }
...
void bubble_sort_timer(void*) {
...

    {
        ...
        while (performed < K && g_bubble_state.active) {
            ...
            if (a > bscore) std::swap(vec[g_bubble_state.j], vec[g_bubble_state.j+1]);
            g_bubble_state.j++;
            performed++;
            any_active = true;
        }
...
// Save sorted results into g_bins and to disk
...
void save_now_cb(Fl_Widget*, void*) { save_sorted_results_to_files(); }
...
void eval_poll_timer(void* userdata) {
    ...
    if (btns) {
        if (btns[0]) btns[0]->activate();
        if (btns[1]) btns[1]->activate();
        if (btns[2]) btns[2]->activate();
    }
...
int main(int argc, char **argv) {
    ...
    Fl_Group* grp2 = new Fl_Group(10, 35, 1240, 645, "Star Sorting (per-LFSR)");
    grp2->begin();
    ...
    userdata[0] = btn_eval; userdata[1] = btn_sort; userdata[2] = btn_save; userdata[3] = nullptr;
