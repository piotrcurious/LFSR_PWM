...
def calculate_lfsr_properties(mask, seed, target_duty_cycle, pwm_period=65536):
    """
    Calculates the LFSR period and actual duty cycle over a full PWM period.
    
    The duty cycle is measured over complete LFSR cycles that fit within the PWM period.
    If the LFSR period doesn't divide evenly into the PWM period, we measure over
    an integer number of complete LFSR cycles to avoid bias from partial cycles.
    
    Returns:
        period: Number of steps before LFSR repeats
        duty_cycle: Ratio of '1' bits over complete LFSR cycles
    ...
    # Step 2: Calculate how many complete cycles fit in the PWM period
    complete_cycles = pwm_period // period
    measurement_steps = complete_cycles * period
    
    # Step 3: Count ones over complete LFSR cycles only
    state = seed
    ones_count = 0
    ...
    # Calculate duty cycle over complete cycles
    duty_cycle = ones_count / measurement_steps
