...
def calculate_lfsr_properties(mask, seed, target_duty_cycle, pwm_period=65536):
    ...
    The duty cycle is measured over the full PWM period (default 65536 steps for 16-bit),
    which may include multiple repetitions of the LFSR sequence.
    
    Returns:
        period: Number of steps before LFSR repeats
        duty_cycle: Ratio of '1' bits over the FULL PWM period
    ...
    # Step 2: Count ones over the FULL PWM period (repeating the LFSR as needed)
    state = seed
    ones_count = 0
    ...
    # Calculate duty cycle over the full PWM period
    duty_cycle = ones_count / pwm_period
