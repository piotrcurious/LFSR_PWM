...
def calculate_lfsr_properties(mask, seed, target_duty_cycle, pwm_period=65536):
    ...
    Uses a weighted average approach (inspired by Kahan summation) to account for
    partial LFSR cycles. The effective duty cycle considers both complete cycles
    and the fractional contribution of the partial cycle at the end.
    
    Returns:
        period: Number of steps before LFSR repeats
        duty_cycle: Effective duty cycle ratio accounting for partial cycles
    ...
    # Step 1: Find the LFSR period and count ones in one complete cycle
    state = seed
    seen_states = {state}
    period = 0
    ones_in_period = 0
    ...
    for step in range(1, pwm_period + 1):
        ...
        if state in seen_states:
            # Entered a cycle that doesn't include seed (bad LFSR)
            period = step
            break
    ...
    if period == 0:
        # Did not find period within pwm_period - measure directly
        period = pwm_period
        duty_cycle = ones_in_period / pwm_period
        duty_error = abs(duty_cycle - target_duty_cycle)
        return period, duty_cycle, duty_error
    
    # Step 2: Calculate the effective duty cycle using weighted average
    complete_cycles = pwm_period // period
    remaining_steps = pwm_period % period
    
    # Count ones in the partial cycle if it exists
    ones_in_partial = 0
    ...
    # Weighted average: (complete cycles × ones_per_cycle + partial cycle ones) / total steps
    total_ones = (complete_cycles * ones_in_period) + ones_in_partial
    duty_cycle = total_ones / pwm_period
    duty_error = abs(duty_cycle - target_duty_cycle)
