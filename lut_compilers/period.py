...
def calculate_lfsr_properties(mask, seed, target_duty_cycle, max_steps=65536):
    """
    Calculates the period and actual duty cycle of an LFSR.
    
    Returns:
        period: Number of steps before repeating (or max_steps if no repeat found)
        duty_cycle: Ratio of '1' bits in the sequence
        duty_error: Absolute difference from target duty cycle
    """
    if seed == 0:
        # Degenerate case: all zeros
        return max_steps, 0.0, target_duty_cycle
    ...
    for step in range(1, max_steps + 1):
        ...
        if state == seed:
            # Found the period
            period = step
            duty_cycle = ones_count / period
            duty_error = abs(duty_cycle - target_duty_cycle)
            return period, duty_cycle, duty_error
        
        if state in seen_states:
            # Entered a cycle that doesn't include the seed (shouldn't happen with good LFSRs)
            period = step
            duty_cycle = ones_count / period
            duty_error = abs(duty_cycle - target_duty_cycle)
            return period, duty_cycle, duty_error
    ...
    # Did not find period within max_steps
    duty_cycle = ones_count / max_steps
    duty_error = abs(duty_cycle - target_duty_cycle)
    return max_steps, duty_cycle, duty_error
