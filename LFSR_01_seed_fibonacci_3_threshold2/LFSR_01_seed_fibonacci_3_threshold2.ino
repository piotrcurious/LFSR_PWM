...
//const uint32_t TICK_HZ = 50000UL;
...
ISR(TIMER1_COMPA_vect) {
  ...
  switch (active_mode) {
    ...
    case MODE_LFSR_PWM: {
      ...
      // Advance LFSR state
 //     uint16_t feedback = __builtin_parity(lfsr_state & lfsr_mask_current) & 1;
...
void loop() {
  ...
  if (now >= next_change_ms) {
    ...
    // or wrap around custom value for debug
   // if (level_index <= 50) { level_index++;} else {level_index = 0;} 
